    ...
    def run(self):
        ...
        try:
            ...
            if info.type.lower() != 'windows':
                ...
                print("erl_crash files under /opt/{0}/var/lib/{0}/"
                      .format(server_type))
                core_files.extend(remote.file_starts_with(
                    "/opt/{0}/var/lib/{0}/".format(server_type),
                    "erl_crash"))
                print("core* files under /opt/{0}/var/lib/{0}/"
                      .format(server_type))
                core_files.extend(remote.file_starts_with(
                    "/opt/{0}/var/lib/{0}/".format(server_type),
                    "core"))
                ...
                print("breakpad *dmp files under /opt/{0}/var/lib/{0}/"
                      .format(server_type))
                core_files.extend(remote.file_ends_with(
                    "/opt/{0}/var/lib/{0}/".format(server_type),
                    ".dmp"))
                if core_files:
                    print("found crashes on %s: %s"
                          % (info.ip, core_files))
                else:
                ...
                for core_file in core_files:
                    if core_file.find('erl_crash.dump') != -1:
                        ...
                        erl_crash_file_name = "erlang-%s-%s.log" \
                                              % (self.server.ip, i)
                        ...
                        breakpad_crash_file_name = "breakpad-%s-%s.dmp" \
                                                   % (self.server.ip, i)
                        remote_path, file_name = os.path.dirname(
                            core_file), os.path.basename(core_file)
                        if remote.get_file(remote_path, file_name,
                        ...
                        command = "/opt/%s/bin/tools/cbanalyze-core" \
                                  % server_type
...
class Clearcoredumps(object):
    ...
    def run(self):
        ...
        try:
            ...
            if info.type.lower() != 'windows':
                core_files = []
                print("looking for Erlang/Memcached crashes on %s"
                      % info.ip)
                core_files.extend(remote.file_starts_with(
                    "/opt/{0}/var/lib/{0}/".format(server_type),
                    "erl_crash"))
                ...
                core_files.extend(remote.file_ends_with(
                    ...
                    ".dmp"))
                if core_files:
                    print("found dumps on {0}: {1}"
                          .format(info.ip, core_files))
                    command = "mkdir -p /tmp/backup_crash/{0};" \
                              "mv -f /tmp/core* /tmp/backup_crash/{" \
                              "0};" \
                              "mv -f /opt/{1}/var/lib/{" \
                              ...
                              "/tmp/backup_crash/{0};". \
                        format(stamp, server_type)
                    print("Moved all dumps on %s to backup folder: "
                          "/tmp/backup_crash/%s" % (self.server.ip, stamp))
                    remote.execute_command(command)
                    output, error = remote.execute_command(
                        "ls -la /tmp/backup_crash/{0}".format(stamp))
                    ...
                    for core_file in core_files:
                        ...
                        if remote.delete_file(remote_path, file_name):
                            print('deleted core file: %s' % core_file)
                    remote.disconnect()
                else:

...
# takes an ini template as input, standard out is populated
# with the server pool need a descriptor as a parameter
...
def main():
    ...
    parser.add_option('-d', '--addPoolServerId',
                      dest='addPoolServerId', default=None)
    parser.add_option('-a', '--addPoolServers',
                      dest='addPoolServers', default=None)
    ...
    if options.addPoolServers is not None \
            and options.addPoolServers != "None":
    ...
    for i in range(len(data)):
        if 'dynamic' in data[i]:
            ...
            data[i] = string.replace(data[i], options.addPoolServerId,
                                     addPoolServers[0])
